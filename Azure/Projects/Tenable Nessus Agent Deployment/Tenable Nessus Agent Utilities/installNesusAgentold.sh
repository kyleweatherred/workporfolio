...
echo "Linking Nessus Agent to the server..." >> "$LOG_FILE"
/opt/nessus_agent/sbin/nessuscli agent link --key="$LINKING_KEY" --name="$AGENT_NAME" --groups="$AGENT_GROUP" >> "$LOG_FILE" 2>&1
...
/opt/nessus_agent/sbin/nessuscli agent link --key="$LINKING_KEY" --cloud >> "$LOG_FILE" 2>&1
