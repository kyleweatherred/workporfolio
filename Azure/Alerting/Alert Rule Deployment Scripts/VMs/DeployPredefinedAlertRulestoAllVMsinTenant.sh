...
create_cpu_alert() {
  ...
  local alert_name="Virtual Machine CPU Utilization Greater Than or Equal to 85 Percent for More Than 5 Minutes -- $vm_name"

  if check_alert_exists "$alert_name" "$rg" "$sub"; then
    echo "Alert '$alert_name' already exists for VM: $vm_name (subscription: $sub). Skipping..."
  else
    # CPU alert uses severity=2 (Warning/Critical level) 
    # and includes BOTH Critical + Informational AGs
    ...
    echo "Created alert '$alert_name' for VM: $vm_name in subscription: $sub."
  fi
}

##############################################
# CREATE ALERT: Availability
##############################################
create_availability_alert() {
  ...
  # Build the descriptive Alert Rule Name
  local alert_name="No Heartbeat for More than 5 Minutes on Virtual Machine -- $vm_name"

  if check_alert_exists "$alert_name" "$rg" "$sub"; then
    echo "Alert '$alert_name' already exists for VM: $vm_name (subscription: $sub). Skipping..."
  else
    # Availability alert uses severity=0 (Highest/Critical)
    # and includes ONLY the Critical AG
    az monitor metrics alert create \
      --name "$alert_name" \
      --resource-group "$rg" \
      --scopes "$vm_id" \
      --condition "avg VmAvailabilityMetric <= 1" \
      --description "No heartbeat detected for 5 minutes. VM might be unavailable." \
      --action "$CRITICAL_ACTION_GROUP_ID" \
      --evaluation-frequency "5m" \
      --window-size "5m" \
      --severity 0 \
...
for sub in $(az account list --query "[?state=='Enabled'].id" -o tsv); do
  ...
  # Determine the correct CRITICAL & NONCRITICAL action group IDs
  ...
  # Gather all VMs in current subscription
  ...
  if [ -z "$vm_ids" ]; then
    echo "No VMs found in subscription $sub. Skipping..."
    continue
  fi

  # Create CPU & Availability Alerts for each VM
  for vm_id in $vm_ids; do
    create_cpu_alert "$vm_id" "$ALERT_RESOURCE_GROUP" "$sub"
    create_availability_alert "$vm_id" "$ALERT_RESOURCE_GROUP" "$sub"
  done
done
