...
tag_vmss() {
  ...
  echo "Tagging VMSS resource: $vmss_id with $tag_key=$tag_value..."
  az resource tag \
    --ids "$vmss_id" \
    --tags "$tag_key=$tag_value" \
    --is-incremental \
    --subscription "$sub" || {
      echo "Failed to tag resource: $vmss_id"
      return 1
    }
  echo "Resource tagged successfully: $vmss_id"
}

##############################################
# Create VMSS Alert Rules
##############################################
create_vmss_alerts() {
  local vmss_id="$1"
  local rg="$2"
  ...
  if ! check_alert_exists "$alert_name" "$rg" "$sub"; then
    ...
    az monitor metrics alert create \
      ...
      --action "$CRITICAL_ACTION_GROUP_ID" || all_alerts_created=false
  fi

  # High CPU Utilization Alert
  alert_name="VMSS CPU Utilization Exceeds 85 Percent for More Than 5 Minutes -- $vmss_name"
  if ! check_alert_exists "$alert_name" "$rg" "$sub"; then
    echo "Creating alert: $alert_name"
    az monitor metrics alert create \
      --name "$alert_name" \
  ...
  if $all_alerts_created; then
    tag_vmss "$vmss_id" "$sub"
  else
    echo "One or more alerts failed for VMSS: $vmss_name. Skipping tagging."
  fi
}
...
for sub in $(az account list --query "[?state=='Enabled'].id" -o tsv); do
  ...
  if [ -z "$vmss_ids" ]; then
    echo "No VMSS found in subscription $sub. Skipping..."
    continue
  fi

  for vmss_id in $vmss_ids; do
    echo "Processing VMSS: $vmss_id"
    create_vmss_alerts "$vmss_id" "$ALERT_RESOURCE_GROUP" "$sub"
  done
done

echo "All VMSS alert rules and tagging completed successfully."
