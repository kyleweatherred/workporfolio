...
##############################################
# CREATE ALERT: SQL Database Availability
##############################################
create_sql_availability_alert() {
  local db_id=$1
  local rg=$2
  local sub=$3
  ...
  local alert_name="SQL Database Availability Less Than 100 Percent for 15 Minutes -- $db_name"

  if check_alert_exists "$alert_name" "$rg" "$sub"; then
    echo "Skipping creation for '$alert_name' as it already exists."
  else
    echo "Creating alert '$alert_name' for SQL Database: $db_name..."
    az monitor metrics alert create \
      --name "$alert_name" \
      --resource-group "$rg" \
      --scopes "$db_id" \
      --condition "avg availability < 100" \
      --description "SQL Database: $db_name availability dropped below 100% for 15 minutes." \
    ...
    if [ $? -eq 0 ]; then
      echo "Tagging resource $db_id with ALERTING=CONFIGURED..."
      az resource tag --ids "$db_id" --tags ALERTING=CONFIGURED --subscription "$sub"
    else
      echo "Failed to tag resource $db_id."
    fi
  fi
}

##############################################
# CREATE ALERT: SQL Database Deadlock
##############################################
create_sql_deadlock_alert() {
  local db_id=$1
  local rg=$2
  local sub=$3
  ...
  local alert_name="SQL Database Deadlock Count Greater Than 0 After 15 Minutes -- $db_name"
  ...
  if check_alert_exists "$alert_name" "$rg" "$sub"; then
    ...
    az monitor metrics alert create \
      ...
      --description "SQL Database: $db_name deadlock count exceeded zero." \
      --action "$CRITICAL_ACTION_GROUP_ID" \
      --evaluation-frequency "1m" \
      --window-size "15m" \
      --severity 1 \
...
##############################################
# CREATE ALERT: SQL Database DTU Utilization
##############################################
create_sql_dtu_alert() {
  local db_id=$1
  local rg=$2
  local sub=$3
  ...
  local alert_name="SQL Database DTU Utilization Exceeds 85 Percent for 15 Minutes -- $db_name"
  ...
  if check_alert_exists "$alert_name" "$rg" "$sub"; then
    ...
    az monitor metrics alert create \
      ...
      --description "SQL Database: $db_name DTU utilization exceeded 85% for 15 minutes." \
      --action "$CRITICAL_ACTION_GROUP_ID" \
      --evaluation-frequency "1m" \
      --window-size "15m" \
      --severity 2 \
...
##############################################
# CREATE ALERT: SQL Database Storage Utilization
##############################################
create_sql_storage_alert() {
  local db_id=$1
  local rg=$2
  local sub=$3
  ...
  local alert_name="SQL Database Storage Utilization Exceeds 90 Percent Capacity -- $db_name"
  ...
  if check_alert_exists "$alert_name" "$rg" "$sub"; then
    ...
    az monitor metrics alert create \
      ...
      --description "SQL Database: $db_name storage utilization exceeded 90% capacity." \
      --action "$CRITICAL_ACTION_GROUP_ID" \
      --evaluation-frequency "1m" \
      --window-size "15m" \
      --severity 2 \
...
for sub in $(az account list --query "[?state=='Enabled'].id" -o tsv); do
  ...
  for db_id in $db_ids; do
    ...
    create_sql_availability_alert "$db_id" "$ALERT_RESOURCE_GROUP" "$sub"
    create_sql_deadlock_alert "$db_id" "$ALERT_RESOURCE_GROUP" "$sub"
    create_sql_dtu_alert "$db_id" "$ALERT_RESOURCE_GROUP" "$sub"
    create_sql_storage_alert "$db_id" "$ALERT_RESOURCE_GROUP" "$sub"
  done
done

echo "All SQL database alert rules deployed successfully."
