...
get_action_groups_for_subscription() {
  ...
  echo "Critical Action Group ID: $CRITICAL_ACTION_GROUP_ID"
  echo "Non-Critical Action Group ID: $NONCRITICAL_ACTION_GROUP_ID"
}
...
tag_front_door() {
  ...
  echo "Tagging Front Door resource: $profile_id with $tag_key=$tag_value..."
  az resource tag \
    --ids "$profile_id" \
    --tags "$tag_key=$tag_value" \
    --is-incremental \
    --subscription "$sub" || {
      echo "Failed to tag resource: $profile_id"
      return 1
    }
  echo "Resource tagged successfully: $profile_id"
}

##############################################
# CREATE ALERT: Front Door/CDN Profile Alerts
##############################################
create_cdn_alerts() {
  local profile_id="$1"
  local rg="$2"
  ...
  local alert_name="Total Request Count Exceeds 3000 for 15 Minutes or Longer on Front Door -- $profile_name"
  if ! check_alert_exists "$alert_name" "$rg" "$sub"; then
    ...
    az monitor metrics alert create \
      ...
      --resource-group "$rg" \
      --scopes "$profile_id" \
      --condition "total RequestCount > 3000" \
      --window-size "PT15M" \
      --evaluation-frequency "PT1M" \
      --description "Total Request Count Exceeds 3000 for 15 Minutes or Longer on Front Door: $profile_name" \
      --severity 1 \
      --action "$NONCRITICAL_ACTION_GROUP_ID" || all_alerts_created=false
  fi

  # High Latency Alert
  alert_name="The Average Front Door Latency Exceeds 5 Seconds for 5 Minutes or More -- $profile_name"
  if ! check_alert_exists "$alert_name" "$rg" "$sub"; then
    echo "Creating alert: $alert_name"
    az monitor metrics alert create \
      --name "$alert_name" \
      --resource-group "$rg" \
      --scopes "$profile_id" \
      --condition "avg TotalLatency >= 5000" \
      --window-size "PT5M" \
      --evaluation-frequency "PT1M" \
      --description "The Average Front Door Latency Exceeds 5 Seconds for 5 Minutes or More: $profile_name" \
      --severity 2 \
      --action "$NONCRITICAL_ACTION_GROUP_ID" || all_alerts_created=false
  fi

  # High 4XX Error Count Alert
  alert_name="85 Percent of all Client Requests for Last 15 Minutes are Returning a 4XX Error Code -- $profile_name"
  if ! check_alert_exists "$alert_name" "$rg" "$sub"; then
    echo "Creating alert: $alert_name"
    az monitor metrics alert create \
      --name "$alert_name" \
      --resource-group "$rg" \
      --scopes "$profile_id" \
      --condition "avg Percentage4XX >= 85" \
      --window-size "PT15M" \
      --evaluation-frequency "PT1M" \
      --description "85 Percent of all Client Requests for Last 15 Minutes are Returning a 4XX Error Code: $profile_name" \
      --severity 3 \
      --action "$NONCRITICAL_ACTION_GROUP_ID" || all_alerts_created=false
  fi

  # High 5XX Error Count Alert
  alert_name="85 Percent of all Client Requests for Last 15 Minutes are Returning a 5XX Error Code -- $profile_name"
  if ! check_alert_exists "$alert_name" "$rg" "$sub"; then
    echo "Creating alert: $alert_name"
    az monitor metrics alert create \
      --name "$alert_name" \
      --resource-group "$rg" \
      --scopes "$profile_id" \
      --condition "avg Percentage5XX >= 85" \
      --window-size "PT15M" \
      --evaluation-frequency "PT1M" \
      --description "85 Percent of all Client Requests for Last 15 Minutes are Returning a 5XX Error Code: $profile_name" \
      --severity 3 \
      --action "$NONCRITICAL_ACTION_GROUP_ID" || all_alerts_created=false
  fi

  if $all_alerts_created; then
    tag_front_door "$profile_id" "$sub" "$rg"
  else
    echo "One or more alerts failed for $profile_name. Skipping tagging."
  fi
}
...
for sub in $(az account list --query "[?state=='Enabled'].id" -o tsv); do
  ...
  if [ -z "$NONCRITICAL_ACTION_GROUP_ID" ] || [ -z "$CRITICAL_ACTION_GROUP_ID" ]; then
  ...
  if [ -z "$profile_ids" ]; then
    echo "No CDN Profiles found in subscription $sub. Skipping..."
    continue
  fi

  for profile_id in $profile_ids; do
    echo "Processing CDN Profile: $profile_id"
    create_cdn_alerts "$profile_id" "$ALERT_RESOURCE_GROUP" "$sub"
  done
done

echo "All Front Door/CDN Profile alert rules and tagging completed successfully."
