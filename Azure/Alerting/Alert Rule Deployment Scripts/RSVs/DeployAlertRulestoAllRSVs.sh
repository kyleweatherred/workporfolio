...
##############################################
# Check if a scheduled query alert already exists
##############################################
check_alert_exists() {
  local alert_name=$1
  local rg=$2
...
##############################################
# CREATE ALERT: Recovery Services Vault Backup Failure
##############################################
create_backup_failure_alert() {
  ...
  if check_alert_exists "$alert_name" "$rg" "$sub"; then
    ...
    echo "Creating alert '$alert_name' for Recovery Services Vault: $rsv_name..."
    az monitor scheduled-query create \
      -g "$rg" \
      -n "$alert_name" \
      --scopes "$rsv_id" \
      --condition "count 'Failed_Backup_Query' > 1 resource id _ResourceId at least 1 violations out of 1 aggregated points" \
      --condition-query "Failed_Backup_Query=AddonAzureBackupJobs | summarize arg_max(TimeGenerated, *) by JobUniqueId | where JobStatus == 'Failed'" \
      --description "Alert when backup jobs fail in the Recovery Services Vault $rsv_name" \
      --evaluation-frequency "PT5M" \
      --window-size "PT5M" \
      --severity 2 \
      --action-groups "$NONCRITICAL_ACTION_GROUP_ID" \
      --tags "ALERTING=CONFIGURED" "ENVIRONMENT=PROD" "PROJECT=ARC"

    if [ $? -eq 0 ]; then
      echo "Tagging resource $rsv_id with ALERTING=CONFIGURED..."
      az resource tag --ids "$rsv_id" --tags ALERTING=CONFIGURED --subscription "$sub"
    else
      echo "Failed to create alert or tag resource $rsv_id."
    fi
  fi
}
...
for sub in $(az account list --query "[?state=='Enabled'].id" -o tsv); do
  ...
  if [ -z "$NONCRITICAL_ACTION_GROUP_ID" ]; then
  ...
  for rsv_id in $rsv_ids; do
    ...
    create_backup_failure_alert "$rsv_id" "$ALERT_RESOURCE_GROUP" "$sub"
  done
done

echo "All Recovery Services Vault alert rules deployed successfully."
